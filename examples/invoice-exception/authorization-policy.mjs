const requirements = Object.freeze({
  stage_resolution: "invoice-resolution:stage",
  commit_resolution: "invoice-resolution:commit",
});
...
export function decideInvoiceAction(request) {
  ...
  if (!request.caller.scopes?.includes(requirements[request.action])) return deny("CALLER_SCOPE_DENIED");
  ...
  if (request.action === "commit_resolution") {
    ...
    if (!Number.isFinite(expiresAt) || !Number.isFinite(now)) return deny("APPROVAL_TIME_INVALID");
